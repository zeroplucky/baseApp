...
public final class KeyboardUtils {
     ...
     * @param view The view.
     */
    public static void showSoftInput(@NonNull Activity activity, @NonNull final View view) {
        showSoftInput(activity, view, 0);
    }
     ...
     */
    public static void showSoftInput(@NonNull Activity activity, @NonNull final View view, final int flags) {
    ...
    private static int getDecorViewInvisibleHeight(@NonNull final Activity activity, @NonNull final Window window) {
    ...
    public static void registerSoftInputChangedListener(@NonNull final Activity activity,
        ...
        registerSoftInputChangedListener(activity, activity.getWindow(), listener);
    }
     ...
     * @param listener The soft input changed listener.
     */
    public static void registerSoftInputChangedListener(@NonNull final Activity activity, @NonNull final Window window,
                                                        @NonNull final OnSoftInputChangedListener listener) {
        ...
        final int[] decorViewInvisibleHeightPre = {getDecorViewInvisibleHeight(activity, window)};
        contentView.getViewTreeObserver()
                .addOnGlobalLayoutListener(new OnGlobalLayoutListener() {
                    @Override
                    public void onGlobalLayout() {
                        int height = getDecorViewInvisibleHeight(activity, window);
    ...
    public static void fixAndroidBug5497(@NonNull final Activity activity) {
        fixAndroidBug5497(activity, activity.getWindow());
    }
     ...
     * @param window The window.
     */
    public static void fixAndroidBug5497(@NonNull final Activity activity, @NonNull final Window window) {
//        int softInputMode = window.getAttributes().softInputMode;
//        window.setSoftInputMode(softInputMode & ~WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE);
        final FrameLayout contentView = window.findViewById(android.R.id.content);
        final View contentViewChild = contentView.getChildAt(0);
        final int paddingBottom = contentViewChild.getPaddingBottom();
        final int[] contentViewInvisibleHeightPre5497 = {getContentViewInvisibleHeight(activity, window)};
        contentView.getViewTreeObserver()
                .addOnGlobalLayoutListener(new OnGlobalLayoutListener() {
                    @Override
                    public void onGlobalLayout() {
                        int height = getContentViewInvisibleHeight(activity, window);
                        if (contentViewInvisibleHeightPre5497[0] != height) {
                            contentViewChild.setPadding(
                                    contentViewChild.getPaddingLeft(),
                                    contentViewChild.getPaddingTop(),
                                    contentViewChild.getPaddingRight(),
                                    paddingBottom + getDecorViewInvisibleHeight(activity, window)
                            );
    ...
    private static int getContentViewInvisibleHeight(@NonNull final Activity activity, final Window window) {
    ...
    private static int getStatusBarHeight(@NonNull final Activity activity) {
        Resources resources = activity.getResources();
        int resourceId = resources.getIdentifier("status_bar_height", "dimen", "android");
        return resources.getDimensionPixelSize(resourceId);
    }

    private static int getNavBarHeight(@NonNull final Activity activity) {
